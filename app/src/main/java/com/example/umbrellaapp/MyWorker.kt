...
import com.example.umbrellaapp.utile.LocationUtile
import com.example.umbrellaapp.utile.WeatherUtile
...
class MyWorker @AssistedInject constructor(
    ...
    companion object {
        ...
        const val WORK_NAME = "com.example.umbrellaapp.MyWorker"
    }
    ...
    override fun doWork(): Result {
        ...
        var firstTimeFlag = inputData.getBoolean("firstTime",true)
        ...
        var isToday = false
        Log.d("ほげ",targetDate.get(Calendar.YEAR).toString())
        ...
        val prefecture = settingInfo.prefecture
        val city = settingInfo.city

        val location = runBlocking{
            locationUtile.getLocation(prefecture = prefecture, city = city)
        }

        val lat = location["lat"]?.toDouble()
        val lon = location["lon"]?.toDouble()
        Log.d("weatherNotification", "lat = $lat, lon = $lon")
    ...
    private suspend fun getDateFromWeekOfDay(dayOfWeek: Int) : Calendar = coroutineScope {
        ...
        Log.d("calender",calendar.get(Calendar.MONTH).toString())
        Log.d("calender",calendar.get(Calendar.THURSDAY).toString())
        var time : String
